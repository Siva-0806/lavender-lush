...
const shareLogic = `window.shareProduct = async function(id, name, imgUrl) {
  if (!navigator.share) {
...
const imageUploadLogic = `document.getElementById('pfImageFile').addEventListener('change', async (e) => {
...
['public/index.html', 'public/collection.html'].forEach(f => {
  ...
  // 1. Add window.shareProduct before async function fetchProducts()
  if (!content.includes('window.shareProduct =')) {
    content = content.replace('async function fetchProducts() {', shareLogic + '\nasync function fetchProducts() {');
  }

  // 2. Add share button in renderProducts
  if (!content.includes('onclick="shareProduct(')) {
    const parts = [
      '<button class="admin-icon-btn" style="margin-right: 5px;" onclick="shareProduct(',
      "'${p.id}', \\`${p.name.replace(/'/g, \"\\\\'\")}\\`, '${p.image}'",
      ')" aria-label="Share"><i data-lucide="share-2" style="width:14px;height:14px;"></i></button>'
    ];
    const shareBtn = parts.join('');
    content = content.replace('<button class="add-cart-btn" data-id="${p.id}">', shareBtn + '\n          <button class="add-cart-btn" data-id="${p.id}">');
  }

  // 3. Update openProductForm to handle image preview display
  if (!content.includes("document.getElementById('pfImagePreview').src = p.image;")) {
  ...
  if (!content.includes("document.getElementById('pfImagePreview').style.display = 'none';")) {
    content = content.replace(
  ...
  // 4. Add imageUploadLogic after productFormOverlay.addEventListener('click', closeProductForm);
  if (!content.includes("document.getElementById('pfImageFile').addEventListener('change'")) {
    content = content.replace(
  ...
  console.log('Features added perfectly to ' + f);
});
